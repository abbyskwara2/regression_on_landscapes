classdef cflLotkaVolterra < communityFunctionLandscape
    ...
    methods 
        ...
        function params = stats(obj)
            % Return LV parameter statistics: mu, sigma, gamma, std K
            % as defined in Bunin PRE 2017
            aij = squareform(tril(obj.inters, -1)); % for i>j, where i=row
            aji = squareform(tril(obj.inters', -1)); % for i<j, and in same order 
            a_offDiag = [aij, aji];
            mu = obj.S * mean(a_offDiag);
            sigma = sqrt(obj.S * var(a_offDiag));
            gamma = corr(aij(:), aji(:));
            stdK = std(obj.K)/mean(obj.K);
            params = [mu, sigma, gamma, stdK];
        end
    end
